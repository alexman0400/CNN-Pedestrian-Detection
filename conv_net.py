...
def cnn_model_fn_6(features, labels, mode):
    # with tf.device('/device:CPU:0'):
        ...
        # caltech dataset images are WIDTHxHEIGHT pixels and 3 colours(RGB), and have one color channel

        # Convolutional Layer #1
        # Computes 32 features using a 5x5 filter with ReLU activation.
        # Padding is added to preserve width and height.
        # Input Tensor Shape: Tensor("conv2d/BiasAdd:0", shape=(batch_size, HEIGHT, WIDTH, 32), dtype=float32)
        # Output Tensor Shape: [batch_size, HEIGHT, WIDTH, 32]

        # Pooling Layer #1
        # First max pooling layer with a 2x2 filter and stride of 2
        # Input Tensor Shape: Tensor("MaxPool2D/MaxPool:0", shape=(?, WIDTH/2, HEIGHT/2, 32), dtype=float32
        # Output Tensor Shape: [?, WIDTH/2, HEIGHT/2, 32]

        # Convolutional Layer #2
        # Computes 32 features using a 5x5 filter with ReLU activation.
        # Padding is added to preserve width and height.
        # Input Tensor Shape: Tensor("conv2d_2/BiasAdd:0", shape=(?, WIDTH/2, HEIGHT/2, 32), dtype=float32)
        # Output Tensor Shape: [?, WIDTH/2, HEIGHT/2, 32]

        # Flattening Layer
        # Input Tensor Shape: [?, WIDTH/2, HEIGHT/2, 32]
        # Output Tensor Shape: Tensor("fully_connected/Relu:0", shape=(?, 1))

        # 1st Fully Connected Layer
        # Input Tensor Shape: [?, WIDTH/2, HEIGHT/2, 32]
        # Output Tensor Shape: Tensor("fully_connected/Relu:0", shape=(?, num_outputs))

        # Dropout Layer
        # Input Tensor Shape: [?, num_outputs]
        # Output Tensor Shape: Tensor("dropout/Identity:0", shape=(?, 1), dtype=float32)

        # 2nd Fully Connected Layer
        # Input Tensor Shape: [?,CLASSES]
        # Output Tensor Shape: Tensor("fully_connected_1/Relu:0", shape=(?, CLASSES), dtype=float32)
        with tf.name_scope('input_layer'):
            input_layer = tf.reshape(features["x"], [-1, WIDTH, HEIGHT, 3])
        ...
        with tf.name_scope('dense_1'):
            dense_1 = tf.layers.dense(inputs=layer_flat, units=num_outputs)
