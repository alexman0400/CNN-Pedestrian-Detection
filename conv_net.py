...
def create_conv_layer(inputs, stride, conv_filter_size, num_filters, relu=False):

    # # Check if RelU layer is needed
    activation = None
    if relu:
...
def cnn_model_fn_2(features, labels, mode):
    ...
    with tf.name_scope('dense_2'):
        dense_2 = tf.layers.dense(inputs=dense_1, units=4096, activation=tf.nn.relu)
...
# # Different models that tried to be trained, but the computer system's GTX did not have enough memory to support them
...
#                                   relu=False)
#     with tf.name_scope('pool3'):
#         pool3 = tf.contrib.layers.max_pool2d(inputs=conv7,
#                                              kernel_size=3,
#                                              stride=1,
#                                              padding='SAME')
#
...
#             drp_1 = tf.layers.dropout(inputs=dense_1)
#
#         with tf.name_scope('dense_2'):
#             dense_2 = tf.layers.dense(inputs=drp_1, units=4096)
#
#         with tf.name_scope('drp_2'):
#             drp_2 = tf.layers.dropout(inputs=dense_2)
#
#         with tf.name_scope('logits'):
